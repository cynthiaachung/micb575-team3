...
#------------------------------------------Ignore section below, not relavent to plots------------------------------------------------
#### Accessor functions ####
...
#------------------------------------------Ignore section above, not relavent to plots------------------------------------------------


#------------------------------------------Do below for phylseq object prep------------------------------------------------
######### ANALYZE ##########
...
#------------------------------------------Do above for phylseq object prep------------------------------------------------




#------------------------------------------Below: Load data if not already in environment------------------------------------------------
#### Load in RData ####
...
# Change LTSP.Treatment to REF, OM1, OM2, OM3
filtered_div_data <- subset(samp_dat_wdiv, LTSP.Treatment == "OM3")
...
if(p_value < 0.05) {
  cat("The difference in Shannon diversity across soil classifications is statistically significant. P-value:", p_value, "\n")
} else {
  cat("The difference in Shannon diversity across soil classifications is not statistically significant. P-value:", p_value, "\n")
}

# Run it! for compaction
gg_richness_compac_horiz <- plot_richness(phylobj_rare, x = "Compaction.Treatment", measures = c("Shannon")) +
  ...
  facet_grid(. ~ LTSP.Treatment, scales = "free_x", space = "free_x") +
  xlab("Compaction Treatment") +
  ggtitle("Alpha Diversity Richness for Compaction Treatment Controlled by LTSP Treatment") +
  theme(
    strip.background = element_blank(),
    strip.placement = "outside",
    strip.text = element_text(size = 14), # Make facet labels larger
...
ggsave("Figure_3_AlphaDiversityRichness_compac_vs_LTSP.png"
       , gg_richness_compac_horiz
       , height=10, width=15)
...
ggsave("Figure_3_AlphaDiversityRichness_compac_vs_LTSP_colorblind.png"
       , gg_richness_compac_horiz_colorblind
       , height=10, width=15)
...
#--------------------------Figure 1 Supplemental below, run entire section--------------------------------------------
...
phylobj_rare_noNAs <- subset_samples(phylobj_rare, !is.na(Soil.Classification) & Soil.Classification != "NA")
pcoa_bc <- ordinate(phylobj_rare_noNAs, method="PCoA", distance=bc_dm)
...
#######Now divide combine soil categories
...
# Change soil classifications to group them
sample_data_df$Soil.Classification[sample_data_df$Soil.Classification %in% c("Aquic Glossudalfs")] <- "Soil_Group1"
sample_data_df$Soil.Classification[sample_data_df$Soil.Classification %in% c("Brunisolic Gray Luvisol", "Orthic Gray Luvisol, Gleyed Gray Luvisol")] <- "Soil_Group2"
sample_data_df$Soil.Classification[sample_data_df$Soil.Classification %in% c("Gleyed Dystric Brunisol","Mesic Ultic Haploxeralfs", "Orthic Dystric Brunisol", "Orthic Humo-Ferric Podzol", "NA")] <- "Soil_Group3"
