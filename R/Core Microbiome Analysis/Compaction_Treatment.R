...
prev_threshold <- 0.05
C0_ASVs <- core_members(mpt_C0, detection=0, prevalence = prev_threshold)
C1_ASVs <- core_members(mpt_C1, detection=0, prevalence = prev_threshold)
C2_ASVs <- core_members(mpt_C2, detection=0, prevalence = prev_threshold)
REF_ASVs <- core_members(mpt_REF, detection=0, prevalence = prev_threshold)

# can plot those ASVs' relative abundance
prune_taxa(C0_ASVs,mpt_RA) %>% 
  plot_bar(fill="Genus") + 
  facet_wrap(.~`Soil.Classification`, scales ="free")
prune_taxa(C1_ASVs,mpt_RA) %>% 
  plot_bar(fill="Genus") + 
  facet_wrap(.~`Soil.Classification`, scales ="free")
prune_taxa(C2_ASVs,mpt_RA) %>% 
  plot_bar(fill="Genus") + 
  facet_wrap(.~`Soil.Classification`, scales ="free")
prune_taxa(REF_ASVs,mpt_RA) %>% 
  plot_bar(fill="Genus") + 
  facet_wrap(.~`Soil.Classification`, scales ="free")
